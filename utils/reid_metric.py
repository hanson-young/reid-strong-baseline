...
class Submit(Metric):
    ...
    def compute(self):
        ...
        print(distmat)
        img_mat = dict()
        print("q ", q_paths.shape)
        print("g ", g_paths.shape)
        for idx in range(q_paths.shape[0]):
            key = q_paths[idx].split('/')[-1]
            img_mat[key] = [g_paths[indices[idx][j]].split('/')[-1] for j in range(self.max_rank)]
